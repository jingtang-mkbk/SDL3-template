...
static void init(AppState *state)
{
  ...
  grid->on_mouseenter = grid_mouseenter;
  grid->on_mouseleave = grid_mouseleave;
...
static void deinit(AppState *state)
{
  ...
  SDL_free(d->back_tex);
  SDL_free(d->restart_tex);
  for (int i = 0; i < arrlen(d->cells); i++)
    free_text(&d->cells[i].text);
  if (d->win_text)
  {
    SDL_free(d->win_text->text);
    SDL_free(d->win_text);
  }
  if (d->game_over_text)
  {
    SDL_free(d->game_over_text->text);
    SDL_free(d->game_over_text);
  }
  SDL_free(d->grid);
